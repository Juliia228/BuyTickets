...
public class RouteRepositoryImpl implements RouteRepository {
    ...
    public Route get(int id) {
        return jdbcTemplate.queryForObject("select * from routes " +
    ...
    public Route save(RouteRequest route) {
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement ps = connection
                    .prepareStatement("insert into routes (departure_point, destination_point, transporter_name, minutes) values (?, ?, ?, ?)",
    ...
    public Route update(Route route) {
        jdbcTemplate.update("update routes set departure_point = ?, destination_point = ?, transporter_name = ?, minutes = ? where id = ?",
                route.getDeparture_point(), route.getDestination_point(), route.getTransporter_name(), route.getMinutes(), route.getId());
        return get(route.getId());
    }

    @Override
    public int delete(int id) {
        return jdbcTemplate.update("delete from routes " +
                "where id = ?", id);
    }
}
